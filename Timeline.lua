...
local Timeline = ReWind:NewModule("Timeline", "AceEvent-3.0")
...
function Timeline:OnEnable()
    self:RegisterMessage("REWIND_ENCOUNTER_END", "OnEncounterEnd")
end
...
function Timeline:OnEncounterEnd(_, encounter)
    if not ReWind.db.profile.timelineAutoShow then return end
    self:Show(encounter)
end
...
function Timeline:Toggle()
    if self.frame and self.frame:IsShown() then
        ...
        local enc = ReWind.state.lastEncounter
        if enc then
            self:Show(enc)
        else
            ReWind:Print("No encounter data to show.")
        end
    end
end
...
function Timeline:GetFrame()
    ...
    local f = CreateFrame("Frame", "ReWindTimeline", UIParent, "BackdropTemplate")
    f:SetBackdrop({
    ...
    f:SetScript("OnDragStop", function(self)
        ...
        ReWind.db.profile.timelinePosition = { point = point, relPoint = relPoint, x = x, y = y }
    end)
    ...
    grip:SetScript("OnMouseDown", function()
        if not ReWind.db.profile.locked then f:StartSizing("BOTTOMRIGHT") end
    end)
    grip:SetScript("OnMouseUp", function()
        f:StopMovingOrSizing()
        ReWind.db.profile.timelineWidth = f:GetWidth()
    end)
    ...
    -- Scroll frame
    local scroll = CreateFrame("ScrollFrame", "ReWindTimelineScroll", f, "UIPanelScrollFrameTemplate")
    ...
    local pos = ReWind.db.profile.timelinePosition
    if pos then
        f:ClearAllPoints()
...
function Timeline:GetRow(parent, index)
    ...
    local breakMarker = row:CreateTexture(nil, "OVERLAY")
    breakMarker:SetSize(ROW_HEIGHT, ROW_HEIGHT)
    breakMarker:SetPoint("RIGHT", row, "RIGHT", 0, 0)
    breakMarker:SetColorTexture(0.9, 0.1, 0.1, 0.15)
    row.breakMarker = breakMarker
...
function Timeline:Render(f, enc)
    ...
    local pct = enc.totalCasts > 0
        and (1 - enc.breaks / enc.totalCasts) * 100 or 100

    f.title:SetText(string.format("ReWind — %dm %02ds — %.1f%% mastery",
        mins, secs, pct))
    ...
    for i, cast in ipairs(enc.casts) do
        ...
        if cast.broke then
            ...
            row.breakMarker:Show()
        else
            row.name:SetTextColor(0.9, 0.9, 0.9)
            row.breakMarker:Hide()
        end
...
function Timeline:GetExportFrame()
    ...
    local ef = CreateFrame("Frame", "ReWindExport", UIParent, "BackdropTemplate")
    ef:SetBackdrop({
        bgFile = "Interface\\Tooltips\\UI-Tooltip-Background",
        edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
        tile = true, tileSize = 16, edgeSize = 12,
    ...
    title:SetFont("Fonts\\FRIZQT__.TTF", 12, "OUTLINE")
    title:SetPoint("TOPLEFT", PADDING, -PADDING)
    title:SetText("ReWind — Export (Ctrl+A, Ctrl+C to copy)")
    title:SetTextColor(0.6, 0.8, 1.0)
    ...
    local scroll = CreateFrame("ScrollFrame", "ReWindExportScroll", ef, "UIPanelScrollFrameTemplate")
...
function Timeline:FormatExport(enc)
    ...
    local lines = {}
    lines[1] = string.format("# ReWind Export — %dm %02ds — %.1f%% mastery (%d casts, %d breaks)",
        mins, secs, pct, enc.totalCasts, enc.breaks)
    lines[2] = "# time,spell,break"

    for _, cast in ipairs(enc.casts) do
        local offset = cast.time - enc.startTime
        local spellInfo = C_Spell.GetSpellInfo(cast.spellId)
        local name = spellInfo and spellInfo.name or tostring(cast.spellId)
        local flag = cast.broke and ",BREAK" or ""
